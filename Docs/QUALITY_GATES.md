...
| UX-11 | 五个主页面只使用系统导航标题，不在内容区重复页面标题；功能栏、内容卡和主要按钮复用统一语义组件。macOS 26+ 只在导航、日期工具、主要操作和临时确认等功能层使用 Liquid Glass，图表和证据内容保持实体表面；旧系统或降低透明度时必须完整回退 | `unifiedFunctionalLayerKeepsOnePageTitleAndSelectiveGlassFallback`、仓库呈现与可访问性回归 |
...
| REVIEW-02 | Codex 写回先给唯一结论、再给唯一步骤；最多两条不重复证据。v7 聚合报告必须带 App 同源的 `attentionTrend`；有进行中实验时 v8 还必须带不含工作流 ID 的 `attentionExperiment`，Codex 只能沿用其问题、动作、证据和验收，不得另起建议。没有进行中实验时可靠纵向结论优先；稳定或改善时不得从低层当天信号另造问题，需关注时不得改写其问题、实验或证据。没有可靠纵向结论且当天数据不可靠时只给修复动作，并兼容已生成的 v1/v2 写回 | `automationReportV7CarriesTheSameAggregateAttentionTrendAsTheApp`、`automationReportV8CarriesOnlyAggregateActiveExperimentProgress`、`codexReviewDecisionBriefRemainsShortAndCompatible`、`codexReviewKeepsLegacyReadCompatibility`、`codexWorkspaceDemandsProblemActionAndNoFiller`、Codex 文件契约测试 |
...
| REVIEW-09 | 回顾页固定沉淀连续工作、碎片化、切换边界、恢复闭环和训练反应五个纵向维度，不生成总分。展示最近 10 个工作日，以最近 3 个可靠日对比此前最多 7 个可靠日；进行中的日期只在说明中展示，不绘制数据点、不参与方向判断。只有恢复后果或两个维度收敛时才升级一个主要问题，并由同一组证据生成一个限时实验；同工作流工具协作不得直接判为碎片问题，明确中断后按时返回原工作流属于恢复闭环而非恢复失败。训练反馈按跨日滚动 5 次判断，没有训练的日期不得重复上一日结果 | `attentionDashboardUsesTenWorkdaysAndLinksTheExperimentToTheMainProblem`、`attentionDashboardExcludesAnUnfinishedDayFromTrendConclusions`、`attentionDashboardDoesNotEscalateToolCollaborationAsFragmentation`、`attentionDashboardTreatsVerifiedReturnAsRecoveryClosureNotFailure`、`attentionDashboardEvaluatesTrainingAcrossDaysInRollingFiveSessions`、`attentionDashboardDoesNotRepeatTrainingResultOnDaysWithoutTraining`、`attentionDashboardDoesNotEscalateFragmentationWithoutAConsequence`、仓库交互回归 |
| REVIEW-10 | Codex 文件桥只在首次接入或用户明确重新登记时写入 App Support；进入回顾页、刷新既有工作区和每日定时复盘不得重复登记。既有生成文件内容未变化时不做原子替换；默认日报只写当前已授权工作区，路径失效时明确要求一次性重新连接 | `codexWorkspaceRefreshReplacesOnlyChangedFiles`、`dailyReportBridgeRegistrationIsExplicitAndOneShot`、`FocusTraceVerification` 文件桥生命周期回归 |
| REVIEW-11 | 可靠主要问题可由用户确认为一项持久实验；同一时间最多一项进行中实验，固定一个行为变量、主指标、目标、最少样本和可选的工作流/时段情境。实验完成或提前结束前不得被次日建议覆盖，也不得自动修改训练计划。只有实际完成对应专注轮次或保存返回点才算执行了变量；评估必须区分无实验机会、未执行或缺少反馈、质量阻断和可靠样本。日维度实验固定首个 10 个完整工作日为证据窗口，趋势滚动不得冲掉已形成的样本，窗口结束仍不足时只能保存为证据不足。恢复闭环优先使用明确等待或被迫中断后 30 分钟内是否回到原工作流，不只依赖手动返回点。报告协议须以 v8 只输出聚合实验进度，并让 Codex 延续同一实验 | `attentionExperimentPersistsOneVariableAndDefersDailyMeasurement`、`attentionExperimentAllowsOnlyOneActiveRecord`、`attentionExperimentSeparatesOpportunityMissingnessAndQualityBlocking`、`attentionExperimentMatchesWorkflowAndTimeBandBeforeComparing`、`attentionExperimentRequiresObservedUseOfTheChangedVariable`、`attentionExperimentTrainingRequiresFiveComparableDifficultySamples`、`attentionExperimentKeepsItsFirstTenDayEvidenceWindow`、`attentionExperimentEndsTenDayWindowAsInsufficientEvidence`、`contextRecoveryUsesNaturalReturnAfterExplicitHandoff`、`localSnapshotDecodesWithoutAttentionExperiments`、`localSnapshotDecodesPersistedAttentionExperiment`、`automationReportV8CarriesOnlyAggregateActiveExperimentProgress`、`FocusTraceVerification` 实验状态与自然恢复回归 |
...
| PERF-10 | 在最近 10 个工作日、2,000 个工作流区间和 1,000 次训练反馈上评估一项同情境注意力实验 | 小于 100 毫秒；沿用回顾缓存 | `FocusTraceVerification` 注意力实验评估性能回归 |

以下属于发版候选版本的本机验收，不能用有噪声的共享 CI 机器冒充精确测量：

- 空闲稳定后 CPU 目标低于 1%；
- 常规工作日数据下稳定 `phys_footprint` 目标低于 100 MB；使用 macOS `footprint` 测量，不把包含共享映射的 RSS 直接当作应用物理占用；
