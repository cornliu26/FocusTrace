...
| **Agent 等待、消息和口头需求插入后，原来的工作回不去** | **不丢**：需求先进入收件箱，再分别确认截止日期、重要程度和所属工作流；Space 与返回点保留原来的工作上下文。 |
...
临时需求先“收下”，不会自动创建、绑定或切换工作流。整理时再分别决定什么时候完成、有多重要、在哪个工作流处理；只有明确点击“开始处理”才会激活它的上下文。
