...
FocusTrace 已经覆盖三条完整路径：

- **看清**：区分工具切换、工作流切换与经过确认的偏离；
- **不丢**：需求先收下，未完成上下文用返回点恢复；
- **改善**：用十日趋势找到一个可靠问题，再用一项持久实验验证一个行为变量。

项目现在进入 **10–15 个真实工作日的效果验证期**，暂不以增加页面、提醒、分数或工作流状态作为默认迭代方向。验证只看维护负担、恢复闭环、冲动切换和建议执行是否真的改善。

最新产品观察是：**工作流是语义承诺，Space 只是上下文线索。** 当前版本仍需要从状态栏绑定桌面，才能形成可靠的自动工作流轨迹；这项绑定不等于专注训练，也不要求为每条需求创建桌面。未来若将它降级为可选自动识别，会作为独立、可回退的交互改造交付，不会在文档里提前假装已经实现。

更完整的依据和实践方法见[《从“切了多少次”到“这次切换是否值得”》](./Docs/SWITCHING_LOAD_PRACTICE.md)，产品边界见[产品纲领](./Docs/PRODUCT_DOCTRINE.md)。
